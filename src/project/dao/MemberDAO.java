...
public interface MemberDAO {
	...
	public boolean checkLogin(String id, String pw); // 로그인시 아이디 비밀번호 확인 : 맞으면 true
	
	public Member select(String id); // 입력된 아이디에 해당하는 모든 정보 출력
	
	public boolean insertRes (Member m); // 진료 예약하기
	
	public List<Member> selectAdmin(); // 관리자 예약출력 (모든 회원정보 불러오기위해 리스트 사용)
	
	public boolean checkRes(String id); //  예약여부확인 (예약하기 버튼 비활성화,예약내역 삭제시) : 예약내역있으면 true
	
	public boolean deleteUser (String id); // <관리자페이지> 테이블뷰에서 선택된 회원 강제탈퇴
